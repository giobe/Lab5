...
public class Ruzzle {
	TreeMap<CharList, ListaPos> presenti = new TreeMap<CharList, ListaPos>();
	...
	public void cercaParole() {
		...
		int passo = 0;
		cerca(sc, passo);
		ArrayList<CharList> key = new ArrayList<CharList>(presenti.keySet());
	...
	public boolean inizialiPresenti(char[] presenti, int passo) {

		if (dao.contiene(presenti, passo) == true) {
			return true;
		} else
			return false;
	}

	public boolean parolaPresente(char[] parola, int passo) {

		if (dao.cerca(parola,passo) == true)
			return true;
		else
			return false;
	}
